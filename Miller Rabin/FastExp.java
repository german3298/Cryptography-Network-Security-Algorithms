...
public class FastExp {
    ...
    public int fastModExponentiation (int base, int exp, int mod) {
        if (mod == 1) return 0;
        int value = 1;
        for (int i = 0; i < exp; i++)
     ...
     */
    public static int fastModExponentiationBit (int base, int exp, int mod) {
        if (mod == 1) return 0;
    ...
    public static void main (String[] args) {
        ...
        s.close();
        int result = FastExp.fastModExponentiationBit(Integer.parseInt(base), Integer.parseInt(exp), Integer.parseInt(mod));
